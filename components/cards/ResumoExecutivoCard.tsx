  ...
  return (
    <div className={`bg-white border border-slate-200 rounded-2xl p-5 shadow-xl`}> 
      <div className="flex items-center justify-between">
        <div>
          <div className={`text-slate-800 font-semibold tracking-tight text-lg`}>Resumo Executivo</div>
          <div className={`text-slate-500 text-sm`}>Indicadores principais do período</div>
        </div>
        <div className="text-right">
          <div className={`text-slate-900 text-xl font-bold tracking-tight`}>{fmt(total)}</div>
      ...
      <div className="grid grid-cols-3 gap-3 mt-4">
        <div className={`bg-slate-50 rounded-xl p-3`}>
          ...
          <div className={`text-slate-900 font-bold tracking-tight text-lg`}>{fmt(pa)}</div>
        </div>
        <div className={`bg-slate-50 rounded-xl p-3`}>
          <div className={`text-slate-600 text-xs`}>RBT12</div>
          <div className={`text-slate-900 font-bold tracking-tight text-lg`}>{fmt(rbt12)}</div>
        </div>
        <div className={`bg-slate-50 rounded-xl p-3`}>
          <div className={`text-slate-600 text-xs`}>Impostos</div>
          <div className={`text-slate-900 font-bold tracking-tight text-lg`}>{Object.keys(trib).filter(k => k !== 'Total' && Number(trib[k]||0)>0).length}</div>
        </div>
      </div>
    </div>
  )
